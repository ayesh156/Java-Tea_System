...
public class ManureService {
    ...
    public List<ManureModel> find(String searchText, int page, int pageSize) {
        ...
        try {
            ...
            if (isNumeric) {
                sql = String.format(
                        "SELECT * FROM manure WHERE supplier_id = '%s' OR supplier_name LIKE '%%%s%%' LIMIT %d, %d",
                        searchText, searchText, offset, pageSize
                );
            } else if (isDate) {
                sql = String.format(
                        "SELECT * FROM manure WHERE date = '%s' OR supplier_name LIKE '%%%s%%' LIMIT %d, %d",
                        searchText, searchText, offset, pageSize
                );
            } else {
                sql = String.format(
                        "SELECT * FROM manure WHERE supplier_name LIKE '%%%s%%' LIMIT %d, %d",
    ...
    public int findCount(String searchText) {
        ...
        try {
            ...
            if (isNumeric) {
                sql = String.format(
                        "SELECT COUNT(*) AS total FROM manure WHERE supplier_id = '%s' OR supplier_name LIKE '%%%s%%'",
                        searchText, searchText
                );
            } else if (isDate) {
                sql = String.format(
                        "SELECT COUNT(*) AS total FROM manure WHERE date = '%s' OR supplier_name LIKE '%%%s%%'",
                        searchText, searchText
                );
            } else {
                sql = String.format(
                        "SELECT COUNT(*) AS total FROM manure WHERE supplier_name LIKE '%%%s%%'",
                        searchText
                );
            }
            ResultSet rs = Mysql.execute(sql);
